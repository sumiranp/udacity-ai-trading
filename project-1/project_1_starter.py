...
def get_top_n(prev_returns, top_n):
    ...
    """
    ## For some reason the dtype setting isnt working
    ans = pd.DataFrame(columns=prev_returns.columns, dtype=np.int64, index=prev_returns.index).fillna(0)
    ans[prev_returns.columns] = ans[prev_returns.columns].astype(int)
#     print(ans)
    for index, row in prev_returns.iterrows():
        date_series = pd.Series(row).nlargest(top_n)
        top_stocks = list(date_series.index.values)
        for stock in ans.columns:
            if(top_stocks and stock in top_stocks):
                ans.loc[index, stock] = 1
#     print(ans)
...
def analyze_alpha(expected_portfolio_returns_by_date):
    ...
    """
    pop_mean = 0
    t, p = stats.ttest_1samp(expected_portfolio_returns_by_date, pop_mean)
    return t, p / 2.0
...
As the p-value is higher than our alpha we cannot reject the NULL hypotheses.
